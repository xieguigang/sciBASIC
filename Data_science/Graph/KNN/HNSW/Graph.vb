#Region "Microsoft.VisualBasic::5aa83e4cb6e3f54adbc7163ce3b3dd9d, Data_science\Graph\KNN\HNSW\Graph.vb"
    ...
    '   Total Lines: 338
    '    Code Lines: 137 (40.53%)
    ' Comment Lines: 152 (44.97%)
    '    - Xml Docs: 40.13%
    ' 
    ...
    '     Class Graph
    ' 
    '         Properties: NewNode, Parameters
    ' 
    '         Constructor: (+1 Overloads) Sub New
    ' 
    '         Function: KNearest, KNearestAtLevel, Print, RandomLevel, Serialize
    ' 
    '         Sub: Create, Deserialize
    ' 
    ' 
    ' 
    ' /********************************************************************************/
...
Namespace KNearNeighbors.HNSW
    ...
    ''' </summary>
    Friend Class Graph(Of TItem, TDistance As IComparable(Of TDistance))

        ''' <summary>
        ''' Gets parameters of the algorithm.
        ''' </summary>
        Private _Parameters As Parameters(Of TItem, TDistance)
        ''' <summary>
        ...
        Public Sub New(distance As Func(Of TItem, TItem, TDistance), parameters As Parameters(Of TItem, TDistance))
            Me.Parameters = parameters
            Select Case Me.Parameters.NeighbourHeuristic
                Case NeighbourSelectionHeuristic.SelectHeuristic
                    NewNode = Function(id, item, level) New NodeAlg4(Of TItem, TDistance)(id, item, level, distance, Me.Parameters)
                Case Else
                    NewNode = Function(id, item, level) New NodeAlg3(Of TItem, TDistance)(id, item, level, distance, Me.Parameters)
            End Select
        End Sub

        Public Property Parameters As Parameters(Of TItem, TDistance)
            Get
                Return _Parameters
            End Get
            ...
            While id < items.Count
                ...
                While level >= 0
                    ...
                    For Each newNeighbour In bestNeighbours
                        ...
                        If SmallWorld(Of TItem, TDistance).DLt(newNode.TravelingCosts.From(newNeighbour), newNode.TravelingCosts.From(bestPeer)) Then
        ...
        Public Function KNearest(destination As Node(Of TItem, TDistance), k As Integer) As IList(Of Node(Of TItem, TDistance))
            Dim bestPeer = entryPoint
        ...
        Friend Function Print() As String
            ...
            While level >= 0
                ...
                SmallWorld(Of TItem, TDistance).BFS(entryPoint, level, Sub(node)
        ...
        Private Shared Function KNearestAtLevel(entryPoint As Node(Of TItem, TDistance), destination As Node(Of TItem, TDistance), k As Integer, level As Integer) As IList(Of Node(Of TItem, TDistance))
            ' 
            ...
            ' prepare tools
            Dim closerIsLess As IComparer(Of Node(Of TItem, TDistance)) = destination.TravelingCosts
            Dim fartherIsLess As IComparer(Of Node(Of TItem, TDistance)) = closerIsLess.Reverse()

            ' prepare heaps
            Dim resultHeap = New BinaryHeap(Of Node(Of TItem, TDistance))(New List(Of Node(Of TItem, TDistance))(k + 1) From {
                    entryPoint
                }, closerIsLess)
            Dim expansionHeap = New BinaryHeap(Of Node(Of TItem, TDistance))(New List(Of Node(Of TItem, TDistance))() From {
                    entryPoint
                }, fartherIsLess)
            ...
            While expansionHeap.Buffer.Any()
                ...
                If SmallWorld(Of TItem, TDistance).DGt(destination.TravelingCosts.From(toExpand), destination.TravelingCosts.From(farthestResult)) Then
                ...
                For Each neighbour In toExpand.GetConnections(level)
                    If Not visited.Contains(neighbour.Id) Then
                        ...
                        If resultHeap.Buffer.Count < k OrElse SmallWorld(Of TItem, TDistance).DLt(destination.TravelingCosts.From(neighbour), destination.TravelingCosts.From(farthestResult)) Then

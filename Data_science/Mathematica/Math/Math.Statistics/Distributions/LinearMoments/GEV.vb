#Region "Microsoft.VisualBasic::26fe19388dc2cd5d9a06cc73ec47e095, Data_science\Mathematica\Math\Math.Statistics\Distributions\LinearMoments\GEV.vb"
...
Namespace Distributions.LinearMoments
    ...
    Public Class GEV : Inherits Distributions.ContinuousDistribution
        ...
        Public Sub New(data As Double())
            ...
            If LM.T3() <= 0 Then 'following works for -0.8 to 0
                ...
                If LM.T3() < -0.8 Then 'use above _k as starting point for newton-raphson iteration to converge to answer
                    If LM.T3() <= -0.97 Then
                        _K = 1 - std.Log(1 + LM.T3()) / std.Log(2) '...unless t3 is below -0.97 in which case start from this formula
                    Else
                        Dim t0 As Double = (LM.T3() + 3) / 2
                        For i As Integer = 1 To 19
                            Dim x2 As Double = std.Pow(2, -_K)
                            Dim x3 As Double = std.Pow(3, -_K)
                            Dim xx2 As Double = 1 - x2
                            Dim xx3 As Double = 1 - x3
                            Dim deriv As Double = (xx2 * x3 * std.Log(3) - xx3 * x2 * std.Log(2)) / std.Pow(xx2, 2)
                            Dim kold As Double = _K
                            _K = _K - (xx3 / xx2 - t0) / deriv
                            If std.Abs(_K - kold) <= _K * 0.000001 Then i = 20
                        Next i
                    End If
                Else
            ...
            If std.Abs(_K) < 0.000001 Then
                _K = 0
                _Alpha = LM.L2() / std.Log(2)
                _Xi = LM.L1() - _Alpha * 0.57721566 'euler's constant
            Else
                Dim gam As Double = std.Exp(SpecialFunctions.gammaln(1 + _K))
                _Alpha = LM.L2() * _K / (gam * (1 - std.Pow(2, -_K)))
        ...
        Public Overrides Function GetInvCDF(probability As Double) As Double
            If _K <> 0 Then
                Return _Xi + (_Alpha / _K) * ((1 - std.Pow((-std.Log(probability)), _K)))
            Else
                Return _Xi - _Alpha * std.Log(-std.Log(probability))
            End If
        End Function

        Public Overrides Function GetCDF(value As Double) As Double
            Return std.Exp(-std.Exp(-Y(value)))
        End Function

        Public Overrides Function GetPDF(value As Double) As Double
            Return (1 / _Alpha) * std.Exp(-(1 - _K) * (Y(value) - std.Exp(-Y(value))))
        End Function

        Private Function Y(value As Double) As Double
            If _K <> 0 Then
                Return (-1 / _K) * std.Log(1 - _K * (value - _Xi) / _Alpha)
            Else

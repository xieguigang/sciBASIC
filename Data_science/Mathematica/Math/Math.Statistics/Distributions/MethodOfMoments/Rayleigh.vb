...
Namespace Distributions.MethodOfMoments
    ...
    Public Class Rayleigh : Inherits Distributions.ContinuousDistribution
        ...
        Public Overrides Function GetInvCDF(probability As Double) As Double
            Return _Sigma * std.Sqrt(-2 * std.Log(probability))
        End Function
        Public Overrides Function GetCDF(value As Double) As Double
            Return 1 - (std.Exp(-(std.Pow(value, 2)) / (2 * (std.Pow(_Sigma, 2)))))
        End Function
        Public Overrides Function GetPDF(value As Double) As Double
            Return (value / (std.Pow(_Sigma, 2))) * std.Exp(-(std.Pow(value, 2)) / (2 * (std.Pow(_Sigma, 2))))
        End Function

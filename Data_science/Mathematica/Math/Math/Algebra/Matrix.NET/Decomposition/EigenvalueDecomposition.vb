#Region "Microsoft.VisualBasic::c6dbf63e175063b63e6f2a5d5b36ffaf, Data_science\Mathematica\Math\Math\Algebra\Matrix.NET\Decomposition\EigenvalueDecomposition.vb"
    ...
    '   Total Lines: 979
    '    Code Lines: 706 (72.11%)
    ' Comment Lines: 140 (14.30%)
    '    - Xml Docs: 42.86%
    ' 
    '   Blank Lines: 133 (13.59%)
    '     File Size: 36.74 KB
...
#Region "Private Methods"
        ...
        Private Sub tql2()
            ...
            For l As Integer = 0 To n - 1
                ...
                tst1 = System.Math.Max(tst1, __std.Abs(m_d(l)) + __std.Abs(m_e(l)))
                Dim m As Integer = l
                While m < n
                    If __std.Abs(m_e(m)) <= eps * tst1 Then
                        Exit While
                    End If
                    m += 1
                End While
                ...
                If m > l Then
                    ...
                    Do
                        ...
                        m_d(l) = c * p
                    Loop While __std.Abs(m_e(l)) > eps * tst1
                End If
                m_d(l) = m_d(l) + f
                m_e(l) = 0.0
            Next
        ...
        Private Sub cdiv(xr As Double, xi As Double, yr As Double, yi As Double)
            ...
            If __std.Abs(yr) > __std.Abs(yi) Then
                r = yi / yr
                d = yr + r * yi
                cdivr = (xr + r * xi) / d
                cdivi = (xi - r * xr) / d
        ...
        Private Sub hqr2()
            ...
            For i As Integer = 0 To nn - 1
                If i < low Or i > high Then
                    ...
                    m_e(i) = 0.0
                End If
                For j As Integer = __std.Max(i - 1, 0) To nn - 1
                    norm = norm + __std.Abs(H(i)(j))
                Next
            Next
            ...
            While n >= low
                ...
                Dim l As Integer = n
                While l > low
                    s = __std.Abs(H(l - 1)(l - 1)) + __std.Abs(H(l)(l))
                    If s = 0.0 Then
                        s = norm
                    End If
                    If __std.Abs(H(l)(l - 1)) < eps * s Then
                        Exit While
                    End If
                    l -= 1
                End While
                ...
                Else
                    ...
                    If iter = 10 Then
                        ...
                        For i As Integer = low To n
                            H(i)(i) -= x
                        Next
                        s = __std.Abs(H(n)(n - 1)) + __std.Abs(H(n - 1)(n - 2))
                        y = 0.75 * s
                        x = y
                        w = (-0.4375) * s * s
                    End If
                    ...
                    While m >= l
                        ...
                        q = H(m + 1)(m + 1) - z - r - s
                        r = H(m + 2)(m + 1)
                        s = __std.Abs(p) + __std.Abs(q) + __std.Abs(r)
                        p = p / s
                        q = q / s
                        r = r / s
                        If m = l Then
                            Exit While
                        End If
                        If __std.Abs(H(m)(m - 1)) * (__std.Abs(q) + __std.Abs(r)) < eps * (__std.Abs(p) * (__std.Abs(H(m - 1)(m - 1)) + __std.Abs(z) + __std.Abs(H(m + 1)(m + 1)))) Then
                            Exit While
                        End If
                        m -= 1
                    End While
                    ...
                    For k As Integer = m To n - 1
                        ...
                        If k <> m Then
                            ...
                            x = __std.Abs(p) + __std.Abs(q) + __std.Abs(r)
                            If x <> 0.0 Then
                                p = p / x
                                q = q / x
                                r = r / x
            ...
            For n = nn - 1 To 0 Step -1
                    ...
                    For i As Integer = n - 1 To 0 Step -1
                        ...
                        Else
                            ...
                            Else
                                ...
                                t = (x * s - z * r) / q
                                H(i)(n) = t
                                If __std.Abs(x) > __std.Abs(z) Then
                                    H(i + 1)(n) = (-r - w * t) / x
                                Else
                                    H(i + 1)(n) = (-s - y * t) / z
                                End If
                ...
                ElseIf q < 0 Then
                    ...
                    If __std.Abs(H(n)(n - 1)) > __std.Abs(H(n - 1)(n)) Then
                        H(n - 1)(n - 1) = q / H(n)(n - 1)
                        ...
                        Else
                            ...
                            Else
                                ...
                                If vr = 0.0 And vi = 0.0 Then
                                    vr = eps * norm * (__std.Abs(w) + __std.Abs(q) + __std.Abs(x) + __std.Abs(y) + __std.Abs(z))
                                End If
                                cdiv(x * r - z * ra + q * sa, x * s - z * sa - q * ra, vr, vi)
                                H(i)(n - 1) = cdivr
                                H(i)(n) = cdivi
                                If __std.Abs(x) > (__std.Abs(z) + __std.Abs(q)) Then
                            ...
                            t = __std.Max(__std.Abs(H(i)(n - 1)), __std.Abs(H(i)(n)))
                            If (eps * t) * t > 1 Then
                                For j As Integer = i To n
                                    H(j)(n - 1) = H(j)(n - 1) / t
                                    H(j)(n) = H(j)(n) / t

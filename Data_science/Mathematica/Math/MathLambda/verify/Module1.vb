...
Module Module1
    ...
    Sub Main()
        ...
        For Each s In {"1 / (1 + sqrt(2))", "1 / sqrt(2)", "1 / (sqrt(a) + sqrt(b))", "a / (b + sqrt(c))", "1 / (x + y * i)"}
            Console.WriteLine($"  rationalize({s}) = {Symbolic.Rationalize(ScriptEngine.ParseExpression(s))}")
        Next

        Console.WriteLine("=== Simplify auto-factor ===")
        For Each s In {"x ^ 2 + 2 * x + 1", "x ^ 2 - 1", "x ^ 3 - x", "x ^ 2 + x", "x + x"}
            Console.WriteLine($"  Simplify({s}) = {Symbolic.Simplify(ScriptEngine.ParseExpression(s))}")
        Next
        ...
        For Each s In {"tan(x)", "sec(x)", "csc(x)", "cot(x)", "sin(x) ^ 2", "cos(x) ^ 2", "sec(x) ^ 2",
            ...
            Console.WriteLine($"  Int {s} = {Symbolic.Integrate(ScriptEngine.ParseExpression(s), "x")}")
        Next

        Console.WriteLine("=== Multivariate factor ===")
        For Each s In {"x ^ 2 - y ^ 2", "x ^ 2 + 2 * x * y + y ^ 2", "x ^ 4 - y ^ 4",
                       "x * y + x", "4 * x ^ 2 - 9 * y ^ 2", "x ^ 2 + y ^ 2"}
            Console.WriteLine($"  Factor({s}) = {Symbolic.Factor(ScriptEngine.ParseExpression(s))}")
        Next
    End Sub
End Module

...
Namespace KMeans
 ...
 _
        ...
        Public Function TreeCluster(Of T As Entity)(source As IEnumerable(Of T),
                                                    ...
                                                    Optional parallelDepth% = 3) As Entity()
            If parallel Then
                Return __firstCluster(source, [stop]:=CInt(source.Count / 2), kmeansStop:=[stop], parallelDepth:=parallelDepth)
            Else
                Return __treeCluster(source, Scan0, CInt(source.Count / 2), kmeansStop:=[stop], parallelDepth:=parallelDepth)
            End If
        End Function

#Region "Microsoft.VisualBasic::2000cf8763618add582da2bcb9f0bbf0, Data_science\Visualization\Visualization\Kmeans\Kmeans.vb"
    ...
    '   Total Lines: 281
    '    Code Lines: 203 (72.24%)
    ' Comment Lines: 49 (17.44%)
    '    - Xml Docs: 93.88%
    ' 
        ...
        Private Function labelSelector(serials As IEnumerable(Of Serial3D), labelsQuantile#) As List(Of Serial3D)
            ...
            serials = serials _
                .Select(Function(s)
                            s.Points = s.Points _
                                .Select(Function(p)
                                            If std.Abs(p.Value.X) >= qX OrElse
                                               std.Abs(p.Value.Y) >= qY OrElse
                                               std.Abs(p.Value.Z) >= qZ Then

...
Imports PrettyPrompt.Highlighting.FormattedString.TextElementsEnumerator
...
'ORIGINAL LINE: public readonly struct AnsiColor : IEquatable<AnsiColor>
Public Structure AnsiColor
	...
	Private Shared ReadOnly ansiColorNames As Dictionary(Of String, AnsiColor) = GetType(AnsiColor).GetFields(BindingFlags.Static Or BindingFlags.Public).Where(Function(f) f.FieldType Is GetType(AnsiColor)).ToDictionary(Function(f) f.Name, Function(f) CType(f.GetValue(Nothing), AnsiColor)!, StringComparer.OrdinalIgnoreCase)

	...
	Friend Module IncrementalRendering
		...
		Private Function CalculateDiffInternal(ByVal currentScreen As Screen, ByVal previousScreen As Screen, ByVal ansiCoordinate As ConsoleCoordinate) As StringBuilder
			...
			For i As Integer = 0 To maxIndex - 1
				...
				If currentCell IsNot Nothing AndAlso currentCell.IsContinuationOfPreviousCharacter Then
					Continue For
				End If
				...
				' handle when we're erasing characters/formatting from the previously ren
				If currentCell Is Nothing OrElse currentCell.Formatting.IsDefault Then
					If Not currentFormatRun.IsDefault Then
						diff.Append(Reset)
						currentFormatRun = ConsoleFormat.None
					End If

					If currentCell?.Text Is Nothing OrElse currentCell.Text = vbLf Then
						diff.Append(" "c)
				...
				If Not currentCell.Formatting.Equals(currentFormatRun) Then
		...
		Private Sub UpdateCoordinateFromCursorMove(ByVal currentScreen As Screen, ByVal ansiCoordinate As ConsoleCoordinate, ByVal diff As StringBuilder, ByRef previousCoordinate As ConsoleCoordinate, ByVal currentCell As Cell)
			Dim characterWidth = If(currentCell Is Nothing, 1, currentCell.ElementWidth)
			...
			If hitRightEdgeOfScreen Then
				If currentCell IsNot Nothing AndAlso Not currentCell.TruncateToScreenHeight Then

...
Namespace Data.Repository
    ...
    ' ============================================================================
    '  TextLineStore.vb —— 与格式无关的纯文本行存储引擎
    '  机制：WAL(写前日志) + 内存虚拟覆盖层(片段表) + 稀疏行索引 + 显式合并
    '  行号约定：全部从 1 开始。
    '  说明：引擎只把每一行视为“不透明文本”，不解析行内容，因此 JSONL / CSV 等
    '        任意按行切分的文本格式都可以复用本引擎。具体格式的行编解码由上层的
    '        行编解码器负责（例如 JSql 的 IRowCodec）。
    ' ============================================================================
    ...
    ''' </summary>
    Public Class TextLineStore
        Implements IDisposable

#Region "常量与嵌套类型"

        Private Const Magic As Integer = &H4A4C5349     ' "JLSI"，保持与旧版一致以兼容既有 .idx
...
#Region "构造 / 生命周期"
        ...
        Public Sub New(dataFilePath As String, Optional options As TextStoreOptions = Nothing)
            ...
            _opt = If(options, New TextStoreOptions())

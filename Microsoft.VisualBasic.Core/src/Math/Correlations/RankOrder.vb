#Region "Microsoft.VisualBasic::c7ff128124736015125badb600e0070e, Microsoft.VisualBasic.Core\src\Math\Correlations\RankOrder.vb"
...
Namespace Math.Correlations
    ...
    Public Structure RankOrder(Of T As IComparable(Of T))
        ...
        ''' <summary>
        ''' tagged input data with the input orders
        ''' </summary>
        ...
        ''' </returns>
        Public Shared Function Ranking(data As IEnumerable(Of RankOrder(Of T)),
                                       Optional desc As Boolean = False,
                                       Optional sort_order As Index(Of T) = Nothing) As RankOrder(Of T)()
            ...
            Else
                If desc Then
                    orders = (From xi As RankOrder(Of T) In data Order By sort_order(xi.value) Descending).ToArray
                Else
                    orders = (From xi As RankOrder(Of T) In data Order By sort_order(xi.value) Ascending).ToArray
                End If
            End If

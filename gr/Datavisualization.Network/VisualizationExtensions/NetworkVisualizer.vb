#Region "Microsoft.VisualBasic::584feca3163c1dae90914eec4a8a2bf4, ..\visualbasic_App\gr\Datavisualization.Network\Datavisualization.Network\Visualize\NetworkVisualizer.vb"
...
Public Module NetworkVisualizer
    ...
    Public Function DrawImage(net As NetworkGraph,
        ...
        margin = If(margin.IsEmpty, New Size(3, 3), margin)

        Return GraphicsPlots(
            frameSize, margin, "white",
            plotAPI:=Sub(ByRef g, region)
                         If backgroundImage Is Nothing Then
                             ...
                             Call g.FillRectangle(br, rect) '绘制背景纹理
                         Else
                             Call g.DrawImage(backgroundImage, 0, 0, frameSize.Width, frameSize.Height)
                         End If
                         ...
                         For Each n As Node In net.nodes  ' 在这里进行节点的绘制
                             ...
                             If displayId Then
                                 ...
                                 If sloci.X < margin.Width Then
                                     sloci = New Point(margin.Width, sloci.Y)
                                 End If
                                 If sloci.Y + size.Height > frameSize.Height - margin.Height Then
                                     sloci = New Point(sloci.X, frameSize.Height - margin.Height - size.Height)
                                 End If
                                 If sloci.X + size.Width > frameSize.Width - margin.Width Then
                                     sloci = New Point(frameSize.Width - margin.Width - size.Width, sloci.Y)
                                 End If

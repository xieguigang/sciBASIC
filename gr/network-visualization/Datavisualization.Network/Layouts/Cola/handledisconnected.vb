...
Class handleDisconnected
	...
	Private packingOptions As Object = New With { _
		Key .PADDING = 10, _
		Key .GOLDEN_SECTION = (1 + Math.sqrt(5)) / 2, _
		Key .FLOAT_EPSILON = 0.0001, _
		Key .MAX_INERATIONS = 100 _
	}
...
End Function)
		...
		Dim corner = New Point() With { _
			Key .x = center.x - g.width / 2, _
			Key .y = center.y - g.height / 2 _
		}
		Dim offset = New Point() With { _
			Key .x = g.x - corner.x + svg_width \ 2 - real_width / 2, _
			Key .y = g.y - corner.y + svg_height \ 2 - real_height / 2 _
		}
...
'     
	...
	Private Function separateGraphs(nodes As any, links As any) As Object
		...
		If is_new Then
			clusters += 1
			graphs.push(New With { _
				Key .array = New Object() {} _
			})
		End If

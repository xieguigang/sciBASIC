...
Class Layout
	...
	Private _groupCompactness As number = 1E-06
...
'     
	...
	Protected Function tick() As Boolean
		If Me._alpha < Me._threshold Then
			Me._running = False
			Me.trigger(New With { _
				Key .type = EventType.[end], _
				Key .alpha = InlineAssignHelper(Me._alpha, 0), _
				Key .stress = Me._lastStress _
			})
			Return True
		End If
		...
		Me.trigger(New With { _
			Key .type = EventType.tick, _
			Key .alpha = Me._alpha, _
			Key .stress = Me._lastStress _
		})
		Return False
	End Function
...
'     
	...
	Private Function flowLayout(Optional axis As String = "y", Optional minSeparation As number = 0) As Layout
		...
		Me._directedLinkConstraints = New With { _
			Key .axis = axis, _
			Key .getMinSeparation = minSeparation _
		}
		Return Me
	End Function
	...
	Private Function flowLayout(Optional axis As String = "y", Optional minSeparation As Func(Of any, number) = Nothing) As Layout
		Me._directedLinkConstraints = New With { _
			Key .axis = axis, _
			Key .getMinSeparation = Function() minSeparation _
		}
		Return Me
	End Function
...
'     
	...
	Private Function alpha(x As number) As Layout
		...
		ElseIf x > 0 Then
			...
			If Not Me._running Then
				Me._running = True
				Me.trigger(New With { _
					Key .type = EventType.start, _
					Key .alpha = InlineAssignHelper(Me._alpha, x) _
				})
				Me.kick()
			End If
		End If
	...
	Private linkAccessor As New LinkLengthTypeAccessor() With { _
		Key .getSourceIndex = AddressOf Layout.getSourceIndex, _
		Key .getTargetIndex = AddressOf Layout.getTargetIndex, _
		Key .setLength = AddressOf Layout.setLinkLength, _
		Key .[getType] = Function(l) If(GetType(_linkType) = [function], Me._linkType(l), 0) _
	}
...
End Function)
		...
		Else
			...
			Me._links.forEach(Function(l) 
			If GetType(l.source) Is number Then
				l.source = Me._nodes(l.source)
			End If
			If GetType(l.target) Is number Then
...
End Function)
		Else
			Me._rootGroup = New With { _
				Key .leaves = Me._nodes, _
				Key .groups = New Object() {} _
			}
		End If
...
End Function)
	...
	Private Sub initialLayout(iterations As number, x As number(), y As number())
		If Me._groups.length > 0 AndAlso iterations > 0 Then
			...
			Dim n = Me._nodes.length
			Dim edges = Me._links.map(Function(e) New With { _
				Key .source = DirectCast(e.source, Node).index, _
				Key .target = DirectCast(e.target, Node).index _
			})
			Dim vs = Me._nodes.map(Function(v) New With { _
				Key .index = v.index _
			})
			Me._groups.forEach(Function(g, i) 
			vs.push(New With { _
				Key .index = InlineAssignHelper(g.index, n + i) _
			})
...
'     
	...
	Private Function routeEdge(edge As any, Optional ah As number = 5, Optional draw As any = Nothing) As any
		...
		'    debugger;
		'}
		Dim vg2 = New TangentVisibilityGraph(Me._visibilityGraph.P, New With { _
			Key .V = Me._visibilityGraph.V, _
			Key .E = Me._visibilityGraph.E _
		})
		Dim port1 = New TVGPoint() With { _
			Key .x = edge.source.x, _
			Key .y = edge.source.y _
		}
		Dim port2 = New TVGPoint() With { _
			Key .x = edge.target.x, _
			Key .y = edge.target.y _
		}
	...
	Private Shared Function getSourceIndex(e As Link(Of Node)) As number
		Return If(GetType(e.source) Is number, CType(e.source, number), DirectCast(e.source, Node).index)
	End Function

	'The link source and target may be just a node index, or they may be references to nodes themselves.
	Private Shared Function getTargetIndex(e As Link(Of Node)) As number
		Return If(GetType(e.target) Is number, CType(e.target, number), DirectCast(e.target, Node).index)
	End Function

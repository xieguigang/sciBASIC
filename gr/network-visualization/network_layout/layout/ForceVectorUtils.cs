	...
	public class ForceVectorUtils
	{
		...
		public static void fcBiRepulsor(Node N1, Node N2, double c)
		{
			...
			if (dist > 0)
			{
				...
				ForceVectorNodeLayoutData N1L = N1.LayoutData;
				ForceVectorNodeLayoutData N2L = N2.LayoutData;

				N1L.dx += (float)(xDist / dist * f);
				N1L.dy += (float)(yDist / dist * f);

				N2L.dx -= (float)(xDist / dist * f);
				N2L.dy -= (float)(yDist / dist * f);
			}
		}
		...
		public static void fcBiRepulsor_y(Node N1, Node N2, double c, double verticalization)
		{
			...
			if (dist > 0)
			{
				...
				N1L.dx += (float)(xDist / dist * f);
				N1L.dy += (float)(verticalization * yDist / dist * f);

				N2L.dx -= (float)(xDist / dist * f);
				N2L.dy -= (float)(verticalization * yDist / dist * f);
			}
		}
		...
		protected internal static double attraction(double c, double dist)
		{
			return 0.01 * -c * dist;
		}

		protected internal static double repulsion(double c, double dist)
		{
			return 0.001 * c / dist;
		}
	}

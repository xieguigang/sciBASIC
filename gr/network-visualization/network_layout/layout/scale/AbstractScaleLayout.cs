...
namespace org.gephi.layout.plugin.scale
{
	...
	/// </summary>
	public abstract class AbstractScaleLayout : AbstractLayout, Layout
	{

		private double scale;
		private Graph graph;

		public AbstractScaleLayout(LayoutBuilder layoutBuilder, double scale) : base(layoutBuilder)
		{
			this.scale = scale;
		}
		...
		public override void goAlgo()
		{
			...
			try
			{
				...
				xMean /= graph.NodeCount;
				yMean /= graph.NodeCount;

				foreach (Node n in graph.Nodes)
				{
					if (!n.Fixed)
					{
						double dx = (n.x() - xMean) * Scale.Value;
						double dy = (n.y() - yMean) * Scale.Value;
		...
		public override LayoutProperty[] Properties
		{
			get
			{
				...
				try
				{
					properties.Add(LayoutProperty.createProperty(this, typeof(Double), NbBundle.getMessage(this.GetType(), "ScaleLayout.scaleFactor.name"), null, "ScaleLayout.scaleFactor.name", NbBundle.getMessage(this.GetType(), "ScaleLayout.scaleFactor.desc"), "getScale", "setScale"));
				}
				catch (Exception e)
				{

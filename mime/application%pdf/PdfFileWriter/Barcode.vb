...
Namespace PdfFileWriter
    ...
    Public Class Barcode
        ...
        ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        Public Overridable Function BarWidth(ByVal Index As Integer) As Integer
    ...
    Public Class Barcode128
        ...
        Public Sub New(ByVal Text As String)
            ...
            If LeadDigitsEnd = TrailDigitsStart AndAlso LeadDigitsCount <> 0 Then
                ...
                For Index = 0 To LeadFnc1End - 1
                    _CodeArray(Math.Min(Threading.Interlocked.Increment(CodeEnd), CodeEnd - 1)) = FNC1
                Next
            ...
            Else
                ...
                If TrailDigitsCount <> 0 Then
                    ' add code set C
                    _CodeArray(Math.Min(Threading.Interlocked.Increment(CodeEnd), CodeEnd - 1)) = CODEC
        ...
        ''' </remarks>
        '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        Public Sub New(ByVal _CodeArray As Integer())
            ' save code array
        ...
        Private Sub EncodeDigits(ByVal TextStart As Integer, ByVal TextEnd As Integer, ByRef CodeEnd As Integer)
            ' convert to pairs of digits
            For Index = TextStart To TextEnd - 1 Step 2
                _CodeArray(Math.Min(Threading.Interlocked.Increment(CodeEnd), CodeEnd - 1)) = 10 * (AscW(Text(Index)) - Asc("0"c)) + (AscW(Text(Index + 1)) - Asc("0"c))
            Next
        ...
        Private Sub EncodeNonDigits(ByVal TextStart As Integer, ByVal TextEnd As Integer, ByRef CodeEnd As Integer)
            ' assume code set B
            Dim CodeSeg = CodeEnd
            _CodeArray(Math.Min(Threading.Interlocked.Increment(CodeEnd), CodeEnd - 1)) = If(CodeSeg = 0, STARTB, CODEB)
            ...
            For Index = TextStart To TextEnd - 1
                ...
                If CurChar < 32 Then
                    ...
                    ' currect segment is code B
                        Case CodeSet.CodeB
                            ...
                            ' one time shift to A
                            _CodeArray(Math.Min(Threading.Interlocked.Increment(CodeEnd), CodeEnd - 1)) = SHIFT
                    ...
                    ' save character
                    _CodeArray(Math.Min(Threading.Interlocked.Increment(CodeEnd), CodeEnd - 1)) = CurChar + 64
                    Continue For
                End If

                ' current character is part of either code set A or code set B
                If CurChar < 96 Then
                    _CodeArray(Math.Min(Threading.Interlocked.Increment(CodeEnd), CodeEnd - 1)) = CurChar - Asc(" "c)
                    Continue For
                End If
                ...
                If CurChar >= AscW(FNC1_CHAR) AndAlso CurChar <= AscW(FNC3_CHAR) Then
                    _CodeArray(Math.Min(Threading.Interlocked.Increment(CodeEnd), CodeEnd - 1)) = If(CurChar = AscW(FNC1_CHAR), FNC1, If(CurChar = AscW(FNC2_CHAR), FNC2, FNC3))
                    Continue For
                End If
    ...
    Public Class Barcode39
        ...
        Public Sub New(ByVal Text As String)
            ...
            ' put * at the begining
            Dim CodePtr = 0
            _CodeArray(Math.Min(Threading.Interlocked.Increment(CodePtr), CodePtr - 1)) = START_STOP_CODE
            ...
            For Index = 0 To Text.Length - 1
                ...
                _CodeArray(Math.Min(Threading.Interlocked.Increment(CodePtr), CodePtr - 1)) = Code
            Next
    ...
    Public Class BarcodeEAN13
        ...
        Public Sub New(ByVal Text As String)
            ...
            For Index = If(Length = 12, 0, 1) To Length - 1
                ...
                _CodeArray(Math.Min(Threading.Interlocked.Increment(CodePtr), CodePtr - 1)) = CodeValue
            Next
    ...
    Public Class BarcodeInterleaved2of5
        ...
        Public Sub New(ByVal Text As String, ByVal Optional AddChecksum As Boolean = False)
            ...
            For Each Chr As Char In Text
                ...
                _CodeArray(Math.Min(Threading.Interlocked.Increment(CodePtr), CodePtr - 1)) = CInt(AscW(Chr) - Asc("0"c))
            Next

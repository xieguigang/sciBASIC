    ...
    Public Class Pdf417Encoder
        ...
        Public Function WidthToHeightRatio(ByVal Ratio As Double) As Boolean
            Try
                ...
                Dim Columns = (-B + Math.Sqrt(B * B + 4 * A * C)) / (2 * A)

                ' calculated data columns and rows to meet the width to height ratio
                ' this calculation adds DataColumns - 1 for rounding up
                Dim dataColumns As Integer = Math.Round(Columns, 0, MidpointRounding.AwayFromZero)
        ...
        Public Function CreateBarcodeMatrix() As Boolean(,)
            ...
            For Index = 0 To PaddingCodewordsCount - 1
                Codewords(Math.Min(Threading.Interlocked.Increment(DataLength), DataLength - 1)) = SwitchToTextMode
            Next
        ...
        Private Sub DataEncoding()
            ...
            If _GlobalLabelIDGeneralPurpose <> 0 Then
                ...
                Dim G2 = Math.DivRem(_GlobalLabelIDGeneralPurpose, 900, G3) - 1
        ...
        Private Sub EncodeNumericSegment(ByVal TotalCount As Integer)
            ...
            While TotalCount > 0
                ...
                Dim SegCount = Math.Min(TotalCount, 44)
                ...
                For Index = 0 To SegCount - 1
                    SegStr.Append(Microsoft.VisualBasic.ChrW(BarcodeBinaryData(Math.Min(Threading.Interlocked.Increment(BarcodeDataPos), BarcodeDataPos - 1))))
                Next
        ...
        Private Sub EncodeTextSegment(ByVal TotalCount As Integer)
            ...
            While TotalCount > 0
                Dim Chr As Integer = BarcodeBinaryData(Math.Min(Threading.Interlocked.Increment(BarcodeDataPos), BarcodeDataPos - 1))
                TotalCount -= 1
...
_Select0_CasePdfFileWriter_Pdf417Encoder_TextEncodingMode_Upper:
        ...
        Private Sub EncodeByteSegment(ByVal Count As Integer)
            ...
            If Count = 1 AndAlso _EncodingMode = EncodingMode.Text Then
                ...
                DataCodewords.Add(BarcodeBinaryData(Math.Min(Threading.Interlocked.Increment(BarcodeDataPos), BarcodeDataPos - 1)))
                Return
            End If
            ...
            If Count >= 6 Then
                While EndPos - BarcodeDataPos >= 6
                    ...
                    Dim Temp = CLng(BarcodeBinaryData(Math.Min(Threading.Interlocked.Increment(BarcodeDataPos), BarcodeDataPos - 1))) << 40 Or CLng(BarcodeBinaryData(Math.Min(Threading.Interlocked.Increment(BarcodeDataPos), BarcodeDataPos - 1))) << 32 Or CLng(BarcodeBinaryData(Math.Min(Threading.Interlocked.Increment(BarcodeDataPos), BarcodeDataPos - 1))) << 24 Or CLng(BarcodeBinaryData(Math.Min(Threading.Interlocked.Increment(BarcodeDataPos), BarcodeDataPos - 1))) << 16 Or CLng(BarcodeBinaryData(Math.Min(Threading.Interlocked.Increment(BarcodeDataPos), BarcodeDataPos - 1))) << 8 Or BarcodeBinaryData(Math.Min(Threading.Interlocked.Increment(BarcodeDataPos), BarcodeDataPos - 1))

                    ' convert to 4 digits base 900 number
                    For Index = 4 To 0 + 1 Step -1
                        DataCodewords.Add(Math.DivRem(Temp, Fact900(Index), Temp))
                    Next

    ...
    Public Class PdfContents
        ...
        Public Sub DrawArc(ByVal ArcStart As PointD, ByVal ArcEnd As PointD, ByVal Radius As SizeD, ByVal Rotate As Double, ByVal Type As ArcType, ByVal OutputStartPoint As BezierPointOne)
            ...
            While Index < SegArray.Length
                DrawBezier(SegArray(Index).X, SegArray(Math.Min(Threading.Interlocked.Increment(Index), Index - 1)).Y, SegArray(Index).X, SegArray(Math.Min(Threading.Interlocked.Increment(Index), Index - 1)).Y, SegArray(Index).X, SegArray(Math.Min(Threading.Interlocked.Increment(Index), Index - 1)).Y)
            End While
        ...
        Public Sub DrawRegularPolygon(ByVal Center As PointD, ByVal Radius As Double, ByVal Alpha As Double, ByVal Sides As Integer, ByVal PP As PaintOp)
            ' validate sides
            If Sides < 3 Then Throw New ApplicationException("Draw regular polygon. Number of sides must be 3 or more")

            ' polygon angle
            Dim DeltaAlpha = 2.0 * Math.PI / Sides
        ...
        Friend Function DrawTextInternal(ByVal Font As PdfFont, ByVal FontSize As Double, ByVal Text As String) As Double           ' font object
            ...
            For Ptr = 0 To Text.Length - 1
                ...
                Else
                    If CharInfo.NewGlyphIndex < 0 Then CharInfo.NewGlyphIndex = If(Font.EmbeddedFont, Math.Min(Threading.Interlocked.Increment(Font.NewGlyphIndex), Font.NewGlyphIndex - 1), CharInfo.GlyphIndex)
        ...
        Friend Function CreateFontResStr(ByVal ResCode As String, ByVal SizeStr As String) As Byte()
            ...
            For Each TextChar In ResCode
                FontRes(Math.Min(Threading.Interlocked.Increment(Index), Index - 1)) = Microsoft.VisualBasic.AscW(TextChar)
            Next

            FontRes(Math.Min(Threading.Interlocked.Increment(Index), Index - 1)) = Microsoft.VisualBasic.AscW(" "c)
            ...
            FontRes(Math.Min(Threading.Interlocked.Increment(Index), Index - 1)) = Microsoft.VisualBasic.AscW(" "c)
            FontRes(Math.Min(Threading.Interlocked.Increment(Index), Index - 1)) = Microsoft.VisualBasic.AscW("T"c)
            FontRes(Math.Min(Threading.Interlocked.Increment(Index), Index - 1)) = Microsoft.VisualBasic.AscW("f"c)
            Return FontRes
        End Function
        ...
        Friend Function DrawTextWithKerning(ByVal Font As PdfFont, ByVal FontSize As Double, ByVal KerningArray As KerningAdjust()) As Double           ' font object
            ...
            While True
                ...
                Width += CInt(Math.Round(KA.Adjust * Font.DesignHeight / 1000.0, 0, MidpointRounding.AwayFromZero))
            End While

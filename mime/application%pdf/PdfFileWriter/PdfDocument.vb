    ...
    Public Class PdfDocument
        ...
        Friend Function TextToByteArray(ByVal Text As String) As Byte()
            ...
            If Not Unicode Then
                ...
                For Each TestChar In Text
                    ByteArray(Math.Min(Threading.Interlocked.Increment(Index), Index - 1)) = Microsoft.VisualBasic.AscW(TestChar)
                Next
            Else
                ...
                For Each TestChar In Text
                    ByteArray(Math.Min(Threading.Interlocked.Increment(Index), Index - 1)) = CByte(AscW(TestChar) >> 8)
                    ByteArray(Math.Min(Threading.Interlocked.Increment(Index), Index - 1)) = Microsoft.VisualBasic.AscW(TestChar)
                Next
            End If

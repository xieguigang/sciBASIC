...
Imports System.Security.Cryptography
    ...
    Public Class PdfEncryption
        ...
        Private Shared ReadOnly PasswordPad As Byte() = {&H28, &HBF, &H4E, &H5E, &H4E, &H75, &H8A, &H41, &H64, &H00, &H4E, &H56, &HFF, &HFA, &H01, &H08, &H2E, &H2E, &H00, &HB6, &HD0, &H68, &H3E, &H80, &H2F, &H0C, &HA9, &HFE, &H64, &H53, &H69, &H7A}
        ...
        Private Function ProcessPassword(ByVal StringPassword As String) As Byte()
            ...
            Dim IndexEnd = Math.Min(StringPassword.Length, 32)
        ...
        Private Sub CreateMasterKey(ByVal UserBinaryPassword As Byte(), ByVal OwnerKey As Byte())
            ...
            Ptr += OwnerKey.Length
            HashInput(Math.Min(Threading.Interlocked.Increment(Ptr), Ptr - 1)) = CByte(Permissions)
            HashInput(Math.Min(Threading.Interlocked.Increment(Ptr), Ptr - 1)) = CByte(Permissions >> 8)
            HashInput(Math.Min(Threading.Interlocked.Increment(Ptr), Ptr - 1)) = CByte(Permissions >> 16)
            HashInput(Math.Min(Threading.Interlocked.Increment(Ptr), Ptr - 1)) = CByte(Permissions >> 24)
        ...
        Private Function CreateEncryptionKey(ByVal ObjectNumber As Integer) As Byte()
            ...
            HashInput(Math.Min(Threading.Interlocked.Increment(Ptr), Ptr - 1)) = CByte(ObjectNumber)
            HashInput(Math.Min(Threading.Interlocked.Increment(Ptr), Ptr - 1)) = CByte(ObjectNumber >> 8)
            HashInput(Math.Min(Threading.Interlocked.Increment(Ptr), Ptr - 1)) = CByte(ObjectNumber >> 16)
            HashInput(Math.Min(Threading.Interlocked.Increment(Ptr), Ptr - 1)) = 0  ' Generation is always zero for this library
            HashInput(Math.Min(Threading.Interlocked.Increment(Ptr), Ptr - 1)) = 0  ' Generation is always zero for this library

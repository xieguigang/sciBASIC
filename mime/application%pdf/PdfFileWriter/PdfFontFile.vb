    ...
    Friend Class PdfFontFile
        ...
        Private Sub GetcmapTable()
            ...
            For Index = 0 To NumberOfTables - 1
                ...
                If SubTbl.Format = 0 Then
                    ...
                    For Code = 0 To 256 - 1
                        SubTbl.GlyphArray(Code) = Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1))
                    Next
        ...
        Private Sub AddCompositeGlyphs(ByVal GlyphList As List(Of CharInfo), ByVal ExtraList As List(Of Integer))
            ...
            For Each GlyphIndex In ExtraList
                ...
                CharInfo.NewGlyphIndex = If(GlyphIndexFont, Math.Min(Threading.Interlocked.Increment(PdfFont.NewGlyphIndex), PdfFont.NewGlyphIndex - 1), GlyphList.Count)
            ...
            For Each CharInfo In GlyphArray
                ...
                If CharInfo.GlyphData IsNot Nothing Then
                    For Each B In CharInfo.GlyphData
                        Checksum += CUInt(B) << 24 - 8 * (Math.Min(Threading.Interlocked.Increment(Ptr), Ptr - 1) And 3)
                    Next
                End If
            Next
        ...
        Private Function ReadInt16BigEndian() As Short
            Return Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1)) << 8 Or Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1))
        End Function
        ...
        Private Function ReadUInt16BigEndian() As UShort
            Return CUInt(Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1))) << 8 Or Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1))
        End Function
        ...
        Private Function ReadUInt32BigEndian() As UInteger
            Return CUInt(Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1))) << 24 Or CUInt(Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1))) << 16 Or CUInt(Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1))) << 8 Or Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1))
        End Function
        ...
        Private Sub WriteInt16BigEndian(ByVal Value As Integer)
            Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1)) = CByte(Value >> 8)
            Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1)) = CByte(Value)
            Return
        End Sub
        ...
        Private Sub WriteUInt32BigEndian(ByVal Value As UInteger)
            Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1)) = CByte(Value >> 24)
            Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1)) = CByte(Value >> 16)
            Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1)) = CByte(Value >> 8)
            Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1)) = CByte(Value)
            Return
        End Sub
        ...
        Private Sub WriteInt64BigEndian(ByVal Value As Long)
            Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1)) = CByte(Value >> 56)
            Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1)) = CByte(Value >> 48)
            Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1)) = CByte(Value >> 40)
            Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1)) = CByte(Value >> 32)
            Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1)) = CByte(Value >> 24)
            Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1)) = CByte(Value >> 16)
            Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1)) = CByte(Value >> 8)
            Buffer(Math.Min(Threading.Interlocked.Increment(BufPtr), BufPtr - 1)) = CByte(Value)

...
Namespace PdfFileWriter
    ...
    ''' <remarks>
    ''' <para>
    ''' For more information go to <ahref="http://www.codeproject.com/Articles/570682/PDF-File-Writer-Csharp-Class-Library-Version#ImageSupport">2.4 Image Support</a>
    ''' </para>
    ''' <para>
    ''' <ahref="http://www.codeproject.com/Articles/570682/PDF-File-Writer-Csharp-Class-Library-Version#DrawImage">For example of drawing image see 3.9. Draw Image and Clip it</a>
    ''' </para>
    ''' </remarks>
    Public Class PdfImage
        ...
        ''' </summary>
        Public CropRect As Rectangle = Rectangle.Empty
...
_Select0_CasePdfFileWriter_SaveImageAs_Jpeg:
        ...
        Friend Function PictureToIndexedImage() As Boolean
            ...
            Dim FrameWidth = Math.Abs(PictureData.Stride)
            ...
            For Y = 0 To HeightPix - 1
                ...
                For X = 0 To WidthPix - 1
                    ...
                    Dim Pixel As Integer = PictureBytes(Math.Min(Threading.Interlocked.Increment(PicPtr), PicPtr - 1)) Or PictureBytes(Math.Min(Threading.Interlocked.Increment(PicPtr), PicPtr - 1)) << 8 Or PictureBytes(Math.Min(Threading.Interlocked.Increment(PicPtr), PicPtr - 1)) << 16
            ...
            ElseIf ColorArray.Count <= 16 Then
                ...
                For Y = 0 To HeightPix - 1
                    ...
                    For X = 0 To WidthPix - 1
                        ...
                        Else
                            ObjectValueArray(Math.Min(Threading.Interlocked.Increment(ObjPtr), ObjPtr - 1)) = ObjectValueArray(Math.Min(Threading.Interlocked.Increment(ObjPtr), ObjPtr - 1)) Or CByte(Index)
                        End If
                    Next
            ...
            Else
                ...
                For Y = 0 To HeightPix - 1
                    ...
                    For X = 0 To WidthPix - 1
                        Dim Pixel As Integer = PictureBytes(Math.Min(Threading.Interlocked.Increment(PicPtr), PicPtr - 1)) Or PictureBytes(Math.Min(Threading.Interlocked.Increment(PicPtr), PicPtr - 1)) << 8 Or PictureBytes(Math.Min(Threading.Interlocked.Increment(PicPtr), PicPtr - 1)) << 16
                        ObjectValueArray(Math.Min(Threading.Interlocked.Increment(ObjPtr), ObjPtr - 1)) = CByte(ColorArray.BinarySearch(Pixel))
                    Next
            ...
            For Index = 0 To ColorArray.Count - 1
                ColorByteArray(Math.Min(Threading.Interlocked.Increment(ColorPtr), ColorPtr - 1)) = CByte(ColorArray(Index) >> 16)
                ColorByteArray(Math.Min(Threading.Interlocked.Increment(ColorPtr), ColorPtr - 1)) = CByte(ColorArray(Index) >> 8)
                ColorByteArray(Math.Min(Threading.Interlocked.Increment(ColorPtr), ColorPtr - 1)) = CByte(ColorArray(Index))
            Next
        ...
        Friend Function PictureToGrayImage() As Boolean
            ...
            For Y = 0 To HeightPix - 1
                ...
                For X = 0 To WidthPix - 1
                    ...
                    Dim Pixel As Integer = (11 * PictureBytes(Math.Min(Threading.Interlocked.Increment(PicPtr), PicPtr - 1)) + 59 * PictureBytes(Math.Min(Threading.Interlocked.Increment(PicPtr), PicPtr - 1)) + 30 * PictureBytes(Math.Min(Threading.Interlocked.Increment(PicPtr), PicPtr - 1)) + 50) / 100
                    ObjectValueArray(Math.Min(Threading.Interlocked.Increment(ObjPtr), ObjPtr - 1)) = CByte(Pixel)
                Next
        ...
        Friend Function PictureToBWImage() As Boolean
            ...
            For Row = 0 To HeightPix - 1
                ...
                For Col = 0 To WidthPix - 1
                    If 11 * PictureBytes(Math.Min(Threading.Interlocked.Increment(PicPtr), PicPtr - 1)) + 59 * PictureBytes(Math.Min(Threading.Interlocked.Increment(PicPtr), PicPtr - 1)) + 30 * PictureBytes(Math.Min(Threading.Interlocked.Increment(PicPtr), PicPtr - 1)) >= Cutoff Then ObjectValueArray(RowPtr + (Col >> 3)) = ObjectValueArray(RowPtr + (Col >> 3)) Or CByte(1 << 7 - (Col And 7))
                Next

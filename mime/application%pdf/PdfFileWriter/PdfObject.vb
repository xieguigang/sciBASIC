...
Namespace PdfFileWriter
    ...
    Public Class PdfObject
        ...
        Private _Document As PdfFileWriter.PdfDocument, _ScaleFactor As Double
        ...
        Friend Function ToPt(ByVal Value As Double) As Single       ' coordinate value in user unit of measure
            ...
            If Math.Abs(ReturnValue) < 0.0001 Then ReturnValue = 0
            Return ReturnValue
        End Function
        ...
        Friend Function Round(ByVal Value As Double) As Single      ' a number to be saved in contents
            If Math.Abs(Value) < 0.0001 Then Value = 0
            Return Value
        End Function
        ...
        Friend Function CompressStream(ByVal InputBuf As Byte()) As Byte()
            ...
            OutputLen += 2
            OutputBuf(Math.Min(Threading.Interlocked.Increment(OutputLen), OutputLen - 1)) = CByte(ReadAdler32 >> 24)
            OutputBuf(Math.Min(Threading.Interlocked.Increment(OutputLen), OutputLen - 1)) = CByte(ReadAdler32 >> 16)
            OutputBuf(Math.Min(Threading.Interlocked.Increment(OutputLen), OutputLen - 1)) = CByte(ReadAdler32 >> 8)
        ...
        Private Function Adler32Checksum(ByVal Buffer As Byte()) As UInteger
            ...
            While Len > 0
                ...
                While Threading.Interlocked.Decrement(n) >= 0
                    AdlerLow += Buffer(Math.Min(Threading.Interlocked.Increment(Pos), Pos - 1))
                    AdlerHigh += AdlerLow
                End While
